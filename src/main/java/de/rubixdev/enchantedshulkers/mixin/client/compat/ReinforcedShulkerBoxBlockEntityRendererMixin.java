...
//$$     @Unique
//$$     private static final List<String> reinforcingMaterials =
//$$         Arrays.stream(ReinforcingMaterialSettings.values())
//$$                 .map(ReinforcingMaterialSettings::getMaterial)
//$$                 .map(ReinforcingMaterial::getName)
//$$                 .toList();
//$$     @Unique
//$$     private static final Map<String, SpriteIdentifier> REINFORCED_CLOSED_SHULKER_TEXTURE_ID_MAP = new HashMap<>() {
//$$         {
//$$             for (String material : reinforcingMaterials) {
//$$                 put(material, new SpriteIdentifier(
//$$                         TexturedRenderLayers.SHULKER_BOXES_ATLAS_TEXTURE, new Identifier(Mod.MOD_ID, "entity/reinforced_shulker/" + material + "/closed_shulker")
//$$                 ));
//$$             }
//$$         }
//$$     };
//$$     @Unique
//$$     private static final Map<String, List<SpriteIdentifier>> REINFORCED_CLOSED_COLORED_SHULKER_TEXTURE_ID_MAP = new HashMap<>() {
//$$         {
//$$             for (String material : reinforcingMaterials) {
//$$                 put(material, COLORS.stream()
//$$                         .map(string -> new SpriteIdentifier(
//$$                                 TexturedRenderLayers.SHULKER_BOXES_ATLAS_TEXTURE,
//$$                                 new Identifier(Mod.MOD_ID, "entity/reinforced_shulker/" + material + "/closed_shulker_" + string)))
...
//$$                 || shulkerBox.getAnimationProgress(f) > 0.01f) return value;
//$$         DyeColor dyeColor;
//$$         return (dyeColor = shulkerBox.getColor()) == null
//$$                 ? REINFORCED_CLOSED_SHULKER_TEXTURE_ID_MAP.get(shulkerBox.getMaterial().getName())
//$$                 : REINFORCED_CLOSED_COLORED_SHULKER_TEXTURE_ID_MAP.get(shulkerBox.getMaterial().getName()).get(dyeColor.getId());
//$$     }
//$$
//$$     @Redirect(
//$$             method = "render(Latonkish/reinfshulker/block/entity/ReinforcedShulkerBoxBlockEntity;FLnet/minecraft/client/util/math/MatrixStack;Lnet/minecraft/client/render/VertexConsumerProvider;II)V",
...
//$$     private void renderClosedBox(ShulkerEntityModel<?> instance, MatrixStack matrixStack, VertexConsumer vertexConsumer, int light, int overlay, float red, float green, float blue, float alpha, ReinforcedShulkerBoxBlockEntity shulkerBox, float f) {
//$$         if (!ClientMod.customModels()
//$$                 || !Utils.shouldGlint(shulkerBox)
//$$                 || shulkerBox.getAnimationProgress(f) > 0.01f) {
...
public abstract class ReinforcedShulkerBoxBlockEntityRendererMixin {}
//#endif
